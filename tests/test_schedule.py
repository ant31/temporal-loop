...
async def test_scheduler_prep(mock_import, mock_client, sample_schedules):
    ...
    assert "wf-1" in scheduler.schedules
    assert scheduler.schedules["wf-1"].state == "created"
    assert isinstance(scheduler.schedules["wf-1"].schedule, Schedule)
    assert "wf-2" in scheduler.schedules
    assert scheduler.schedules["wf-2"].state == "paused"
    assert "wf-3" in scheduler.schedules
    assert scheduler.schedules["wf-3"].state == "deleted"
    assert scheduler.schedules["wf-3"].schedule is None
...
async def test_scheduler_prep_with_import_error(mock_import, mock_client, sample_schedules):
    ...
    assert "wf-invalid" not in scheduler.schedules
