...
def test_import_from_string_invalid_format():
    ...
    with pytest.raises(
        ImportFromStringError, match='Import string "invalid" must be in format "<module>:<attribute>".'
    ):
...
def test_import_from_string_module_not_found():
    ...
    with pytest.raises(ImportFromStringError, match='Could not import module "non_existent_module".'):
...
def test_import_from_string_attribute_not_found():
    ...
    with pytest.raises(
        ImportFromStringError, match='Attribute "non_existent_function" not found in module "temporalloop.utils".'
    ):
