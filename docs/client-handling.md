...
When you run `temporalloop`, it automatically creates a single `temporalio.client.Client` instance that is shared among all the workers running in that process. This is handled internally by the `Looper` and `WorkerFactory`.
